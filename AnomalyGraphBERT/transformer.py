...
from graph_attention import MultiHeadGraphAttentionLayer

from timm.models.layers import trunc_normal_

from functions import clone_layer
...
class EncoderLayer(nn.Module):
    def __init__(self, attention_layer, feed_forward_layer, norm_layer, max_seq_len, dropout=0.1):
        ...
        self.relative_position = nn.Parameter(torch.arange(self.attention_layer.n_nodes).to(torch.float).view(1, -1, 1))

        from graph_attention import build_square_matrix
        self.adj = build_square_matrix(self.attention_layer.n_nodes)
    ...
    def forward(self, x):
        ...
        x = self.attention_layer(x, self.adj)

        return self.dropout_layer(self.norm_layers[1](self.feed_forward_layer(x))) + x
...
def get_transformer_encoder(n_nodes,
                            in_features,
                            out_features=512,
    ...
    attention_layer = MultiHeadGraphAttentionLayer(n_heads=n_heads,
                                                   n_nodes=n_nodes,
